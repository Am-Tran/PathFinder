...
def main():
    ...
    try:       
        ...
        try:
            ...
            # On définit nos updaters
            taches_updaters = {
                "UPDATER_FT": [os.path.join(PROJECT_ROOT, "scrapers", "francetravail", "updater_francetravail.py")],
                "UPDATER_APEC": [os.path.join(PROJECT_ROOT, "scrapers", "apec", "updater_apec.py")],
                "UPDATER_WTTJ": [os.path.join(PROJECT_ROOT, "scrapers", "wttj", "updater_wttj.py")],
            }
            
            for source, scripts in taches_updaters.items():
                t = threading.Thread(target=run_chain, args=(source, scripts))
                threads_updaters.append(t)
                t.start()
